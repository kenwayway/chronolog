    ...
    return (
        ...
        >
            <div style={{
                ...
                <div onMouseDown={(e) => e.stopPropagation()} style={{ pointerEvents: 'auto' }}>
                    {children}
                </div>
            </div>
        </div>
    ...
    const renderInputForm = (inFocusMode = false) => (
        <div
            ...
            <div className="flex items-stretch" style={{ backgroundColor: 'var(--bg-primary)' }}>
                ...
                {/* Editor */}
                <div style={{ flex: 1, padding: inFocusMode ? '24px 16px 24px 16px' : '12px 12px 12px 12px' }}>
                    <textarea
            ...
            {(imageUrl || location || showImageInput || showLocationInput) && (
                <div style={{ padding: '8px 16px', borderTop: '1px solid var(--border-subtle)', backgroundColor: 'var(--bg-secondary)' }}>
                    {/* Image URL Input */}
                    {showImageInput && (
                        <div className="flex items-center gap-2 mb-2">
                            <Image size={14} style={{ color: 'var(--text-dim)', flexShrink: 0 }} />
                            <input
                                type="text"
                                value={imageUrl}
                            ...
                            <button
                                ...
                                style={{ color: 'var(--text-dim)', backgroundColor: 'transparent', border: 'none', cursor: 'pointer', fontSize: 16 }}
                            >×</button>
                        </div>
                    )}
                    ...
                    {imageUrl && !showImageInput && (
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            />
                            <span style={{ fontSize: 11, color: 'var(--text-dim)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{imageUrl}</span>
                            <button
                                onClick={() => setImageUrl('')}
                                style={{ color: 'var(--text-dim)', backgroundColor: 'transparent', border: 'none', cursor: 'pointer', fontSize: 14 }}
                            >×</button>
                        </div>
                    )}
                    ...
                    {location && !showLocationInput && (
                        <div className="flex items-center gap-2">
                            ...
                            <span style={{ fontSize: 11, color: 'var(--text-secondary)', flex: 1 }}>{location}</span>
                            <button
                                onClick={() => setLocation('')}
                                style={{ color: 'var(--text-dim)', backgroundColor: 'transparent', border: 'none', cursor: 'pointer', fontSize: 14 }}
                            >×</button>
            ...
            <div className="flex-between" style={{ padding: '10px 16px', borderTop: '1px solid var(--border-subtle)', backgroundColor: 'var(--bg-secondary)' }}>
                <div className="flex items-center gap-2">
                    {/* Status */}
                    <div className="flex items-center gap-2" style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'monospace', marginRight: 8 }}>
                        <span style={{
                            width: 6,
                            height: 6,
                            borderRadius: '50%',
                        ...
                        <span className="uppercase tracking-wider hide-mobile">{isStreaming ? 'SESSION ACTIVE' : 'READY'}</span>
                    </div>
