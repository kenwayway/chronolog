...
interface UseAIReturn {
    suggestCategory: (input: string, categories: Category[]) => Promise<SuggestCategoryResult>
    loading: boolean
    error: string | null
}
...
export function useAI(config: AIConfig = {}): UseAIReturn {
    ...
    const suggestCategory = useCallback(async (input: string, categories: Category[]): Promise<SuggestCategoryResult> => {
        ...
        try {
            ...
            if (jsonMatch) {
                const result = JSON.parse(jsonMatch[0]) as { categoryId?: string; confidence?: number }
                // Validate that the category exists
                const validCategory = categories.find(c => c.id === result.categoryId)
                return {
                    categoryId: validCategory ? result.categoryId as CategoryId : null,
